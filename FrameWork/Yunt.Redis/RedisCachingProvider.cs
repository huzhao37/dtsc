    ...
    public class RedisCachingProvider : MarshalByRefObject, IRedisCachingProvider
    {
        ...
        private void SelectDB(TcpClient client)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Error("堆栈内存分配不足：{0}", ex.Message);
            }
        ...
        public T HashGet<T>(string key, string name, DataType type)
        {
            ...
            using (var c = GetReader())
            {
                ...
                using (var cmd = new Command())
                {
                    ...
                    using (var r = TcpClient.Send(cmd, c.Client))
                    {
                        if (r.ResultDataBlock != null && r.ResultDataBlock.Any())
                        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public List<string> HashGetAllFields(string key, DataType type)
        {
            ...
            using (var c = GetReader())
            {
                ...
                using (var cmd = new Command())
                {
                    ...
                    using (var r = TcpClient.Send(cmd, c.Client))
                    {
                        if (r.ResultDataBlock?.Count > 0)
                        {
                            result.AddRange(r.ResultDataBlock.Select(t => (string)FromRedis(t, type, typeof(string))));
                        }
                    }
                }
            }
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public List<T> HashGetAllValuesT<T>(string key, DataType type)
        {
            var result = new List<T>();
            using (var c = GetReader())
            {
                ...
                using (var cmd = new Command())
                {
                    ...
                    using (var r = TcpClient.Send(cmd, c.Client))
                    {
                        if (r.ResultDataBlock.Count > 0)
                        {
                            foreach (var x in r.ResultDataBlock)
                            {
                                var re = (List<T>)FromRedis(x, type, typeof(List<T>));
                                if (re != null) result.AddRange(re);
                            }
                            // result.AddRange(r.ResultDataBlock.Select(t => (List<T>)FromRedis(t, type, typeof(List<T>))));
                        }
                    }
                }
            }
        ...
        //private List<T> GetResultSpaceT<T>(int count)
        //{
        //    var result = new List<T>(count);
        //    for (var i = 0; i < count; i++)
        //    {
        ...
        private object FromRedis(ArraySegment<byte> data, DataType type, Type otype)
        {
            ...
            else
            {
                ...
                {
                    Logger.Error($"{otype} type get error!", exception);
                    return null;
                }
            }
        }
        ...
        ///     默认客户端
        /// </summary>
        public static RedisCachingProvider DefaultDB { get; }
        ...
        {     
            ...
            if (_options.SingleMode)
            {
                    ...
                    foreach (var hostStrings in _options.RedisServer)
                    {
                        ...
                        WriteHosts.Add(new RedisHost(hostStrings.Host, _options.Connections));
                        ReadHosts.Add(new RedisHost(hostStrings.Host, _options.Connections));

                    }
            }
            else
            {
                foreach (var hostStrings in _options.Writes)
                {
                    WriteHosts.Add(new RedisHost(hostStrings.WriteHost, hostStrings.Connections));
                }
                foreach (var hostStrings in _options.Reads)
                {
                    ReadHosts.Add(new RedisHost(hostStrings.ReadHost, hostStrings.Connections));
                }
            }
        ...
        {
            DefaultDB = new RedisCachingProvider();
        }
        ...
        /// <returns></returns>
        public static RedisCachingProvider GetClient(RedisCachingProvider db)
        {
            return db ?? DefaultDB;
        }
